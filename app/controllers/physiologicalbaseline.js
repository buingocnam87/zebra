...
var PhysiologicalBaseLine = mongoose.model('PhysiologicalBaseLine');
...
execAPI(params, req, res, function(){
	async.each(arrs, function (arr, next) {
			...
			PhysiologicalBaseLine.create(ps_create, function (error, newPS) {
				if (error) {
			...
			async.each(scores, function (score, next) {
				...
				if (data_out == undefined){
					...
					*/
					...
					{
						data_out.hr = (parseFloat(result.hr) + parseFloat(score.hr));
						data_out.count_hr +=1 ;
					}
					if(parseFloat(score.rr)>0)
					{
						data_out.rr = (parseFloat(result.rr) + parseFloat(score.rr));
						data_out.count_rr +=1;
					}
					data_out.bl = (parseFloat(result.bl) + parseFloat(score.bl));
					data_out.temp = (parseFloat(result.temp) + parseFloat(score.temp));
					if(parseFloat(score.spo2)>0)
					{
						data_out.spo2 = (parseFloat(result.spo2) + parseFloat(score.spo2));
						data_out.count_spo2 +=1;
					}
					data_out.bm = (parseFloat(result.bm) + parseFloat(score.bm));
				}
				next();
			}, function (error) {
				console.log('Hellonn');
				if (error) {
					...
					//for (var i = 0; i < n; i++) {
						if (data_out!= undefined) {
							...
							data_out.hr = data_out.hr/data_out.count_hr;
							data_out.rr = data_out.rr/data_out.count_rr;
							data_out.bl = data_out.bl/count_;
							data_out.temp = data_out.temp/count_;
							data_out.spo2 = data_out.spo2/data_out.count_spo2;
							data_out.bm = data_out.bm/count_;
							outPut.push(data_out);
						} else {
							outPut.push({
										"hr":"0",
										"rr":"0",
